...
projet de première année sous l'encadrement de Jérôme Ermont. Le but de
...
données dans les réseaux informatiques, permet de rediriger les packets
...
Ainsi il s'agissait dans ce projet de simuler le travail d'un élément
...
Nous avons ensuite regroupé notre travail et avons modifié la structure
générale afin de l'optimiser au maximum. 

Enfin, il faut remarquer que notre application est vouée à évoluer : il
...
décomposer en module indépendant à l'image des couches du modèle OSI. De
...
commande. La simulation est commandé par le fichier d'entrée, comportant
des instructions \texttt{IN} et des instruction \texttt{out}, dictant
l'entrée ou la sortie d'un packet.
...
nommé \texttt{ListPacketFrag}, en attandant d'obtenir toute les trames
...
de trame, remplit au fur et à mesure de leur arrivée. Lorsque toute les
trames d'un même paquet sons reçu, le paquet fragmenté est retiré de la
...
le fragmenter, avant de l'envoyer dans la \texttt{queue} dédié au lien
de sortie précédement établit.

Lors d'une instruction \texttt{OUT}, le module \texttt{queue} est appelé
afin d'envoyer une trame sur le lien de sortie concernée et donc de
...
dans l'application ainsi que les relations qui les lie.
...
nos yeux une trame définie comme ci dessous:
\begin{minted}[linenos,tabsize=4]{c}
 struct Packet {
  unsigned int label;
...
s'occupe de l'ouverture des différents fichier. Il s'occupe également de
charger la table de commutation par le biais du module \texttt{commut}.
Tous comme \texttt{trame\_decode}, il implémente une machine à état pour
...
l'envoyer dans la structure de stoquage temporaire en attendant
l'assemblage. S'il s'agit de la dernière trame manquante d'un paquet,
les trames associées sont retiré de la liste pour être assemblé. Le
...
file d'attente associé au lien de sortie demandé. La trame ainsi
récupéré est écrite dans le bon fichier par le biais de 
\texttt{trame\_encode}.
